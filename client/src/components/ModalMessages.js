		...
		>
			<div className="modal-dialog modal-dialog-centered" role="document">
				<div className="modal-content">
					...
					<div className="modal-body">
						<div className="alert">{textModal}</div>
					</div>
...
const CreatePageModal = (props) => {
	...
	const getCtgrField = (ctgrIdPage, nameField) => {
		let valueField = null;
		if (nameField === 'ctgrBGC') {
			const item = categories.find((param) => param._id === ctgrIdPage);
			valueField = item.catBGC;
		} else if (nameField === 'ctgrClass') {
			const item = categories.find((param) => param._id === ctgrIdPage);
			valueField = item.catName;
		} else if (nameField === 'ctgrColor') {
			const item = categories.find((param) => param._id === ctgrIdPage);
			valueField = item.catColor;
		}
		return valueField;
	};
	...
	if (categories.length !== 0) {
		categoriesList = categories.map((item, index) => {
			return (
				<option key={index} className={item.catClass} value={item._id}>
					{item.catName}
				</option>
			);
		});
					...
					<div className="modal-body">
						...
						>
							<div className="form-label-group">
								...
								/>
								<div className="invalid-feedback">{FEEDBACK_TEXT}</div>
							</div>
							...
							<div className="form-label-group">
								...
								<input
									...
									aria-describedby="addPage"
								/>
								<div className="invalid-feedback">{FEEDBACK_LINK}</div>
							</div>
							...
							<button
								...
								onClick={(ev) => {
									...
									const ctgrIdPage = $('#ctgrIdPage').val();
									const obj = {
										namePage: $('#namePage').val(),
										ctgrBGC: getCtgrField(ctgrIdPage, 'ctgrBGC'),
										ctgrClass: getCtgrField(ctgrIdPage, 'ctgrClass'),
										ctgrColor: getCtgrField(ctgrIdPage, 'ctgrColor'),
										linkPage: $('#linkPage').val(),
										ctgrIdPage: ctgrIdPage,
										userId: userID,
										screenPage: $('#screenPage').val(),
										orderNum: orderNum,
									};
									validateForm(obj, ev.target.name);
								}}
							>
								Create Page
							</button>
					...
					<div className="modal-body">
						...
						>
							...
							<div className="form-label-group">
								...
								<input
									value={
										pageDetails.screen == null ? '' : pageDetails.screen
									}
									onChange={(ev) => {
										const pattern = PATTERN_LINK;
										isValideField(ev.target.id, pattern);
							...
							<div className="form-label-group">
								...
								<select
									value={
										pageDetails.ctgrId == null ? '' : pageDetails.ctgrId
									}
									onChange={(ev) =>
										handlerInputsValue(ev.target.value, ev.target.id)
									}
							...
							<div className="form-label-group">
								...
								<input
									value={
										pageDetails.orderNum == null
											? ''
											: pageDetails.orderNum
									}
									onChange={(ev) =>
										handlerInputsValue(ev.target.value, ev.target.id)
									}
...
const CreateUserModal = (props) => {
	const { isValideField, validateForm, handlerInputsValue } = props;
...
const ResetPasswordModal = (props) => {
	const { validateForm, isValideField, handlerInputsValue } = props;
